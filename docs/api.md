...
- `options` <[Object]>
  ...
  - `clickCount` <[number]> defaults to 1. See [UIEvent.detail].
  - `delay` <[number]> Time to wait between `mousedown` and `mouseup` in milliseconds. Defaults to 0.
  - `offset` <[Object]> Offset in pixels relative to the border box of the element.
    - `x` <number> x-offset in pixels relative to the border box of the element.
    - `y` <number> y-offset in pixels relative to the border box of the element.
